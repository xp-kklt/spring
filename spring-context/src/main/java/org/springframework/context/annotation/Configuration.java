...
public @interface Configuration {
	...
	/**
	 * 显式指定与 @Configuration 关联的 Spring bd的name。
	 * 如果未指定（常见情况），将自动生成一个 bean name。
	 * <p>
	 * Explicitly specify the name of the Spring bean definition associated with the
	 * {@code @Configuration} class. If left unspecified (the common case), a bean
	 * name will be automatically generated.
	 * <p>
	 * 自定义名称仅在 @Configuration 类通过组件扫描获取或直接提供给
	 * {@link AnnotationConfigApplicationContext} 时适用。
	 * 如果 @Configuration 类注册为传统的 XML bean 定义，则 bean 元素的 name/id 将优先。
	 *
	 ...
	 * 例如即使在用户代码中直接调用 {@code @Bean} 方法的情况下，也能返回共享的单例 bean 实例。
	 * 这个特性需要方法拦截，通过一个运行时生成的CGLIB子类实现，
	 * 这个子类有一些限制，比如配置类及其方法不允许声明{final}。
	 * <p>
	 ...
	 * <p>
	 * 默认为true，允许配置类内部的引用，同样适用于外部调用这个配置类的@Bean方法，比如从另一个配置类调用这个。
	 * 如果由于此特定配置的每个 {@code @Bean} 方法都是自包含的并且设计为容器使用的普通工厂方法而不需要这样做，
	 * 请将此标志切换为 {@code false} 以避免 CGLIB 子类处理。
	 * <p>关闭 bean 方法拦截可以有效地单独处理 {@code @Bean} 方法，
	 * 就像在非 {@code @Configuration} 类上声明时一样，
	 * 又名“@Bean Lite 模式”（参见 {@link Bean @Bean 的 javadoc}） .
