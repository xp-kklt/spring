...
public abstract class ScopedProxyUtils {
	...
	/**
	 * 为提供的目标 bean 生成scoped proxy，使用内部名称注册目标 bean 并在scoped proxy上设置“targetBeanName”。
	 * <p>
	...
	public static BeanDefinitionHolder createScopedProxy(BeanDefinitionHolder definition,
		...
		// 为原始 bean name创建一个scoped proxy definition，将目标 bean“隐藏”在内部目标定义中。

		// beanClass设置为ScopedProxyFactoryBean
		RootBeanDefinition proxyDefinition = new RootBeanDefinition(ScopedProxyFactoryBean.class);
		// 设置目标bean,这里目标bean应该是被代理的bean
		proxyDefinition.setDecoratedDefinition(new BeanDefinitionHolder(targetDefinition, targetBeanName));
		proxyDefinition.setOriginatingBeanDefinition(targetDefinition);
		proxyDefinition.setSource(definition.getSource());
		proxyDefinition.setRole(targetDefinition.getRole());
		// 增加一个属性targetBeanName
