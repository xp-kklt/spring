...
public abstract class AutoProxyUtils {
	...
	/**
	 * bd属性，可以指示是否应该用其目标类代理给定的 bean（以防它首先被代理）。
	 * 值为 {@code Boolean.TRUE} 或 {@code Boolean.FALSE}。
	 * 如果代理工厂为特定 bean 构建了目标类代理，并且希望强制该 bean 始终
	 * 可以强制转换为其目标类（即使 AOP advices通过自动代理应用），则可以设置此属性。
	 * <p>
	...
	/**
	 * 指示自动代理 bean 的原始目标类的 bd 属性，例如用于对基于接口的代理背后的目标类的注解进行内省。
	 * <p>
	...
	static void exposeTargetClass(
		...
		// 判断bd中是否有该beanName
		// 还是在bd中设置属性ORIGINAL_TARGET_CLASS_ATTRIBUTE
		if (beanName != null && beanFactory.containsBeanDefinition(beanName)) {
