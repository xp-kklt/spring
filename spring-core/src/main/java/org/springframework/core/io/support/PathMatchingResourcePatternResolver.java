...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 返回此资源模式解析器使用的 PathMatcher。这里包括路径分割符/
	 * <p>
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			// 一个类路径资源（可能有多个同名资源）
			// 这里其实应该是截取*之前的部分，以便为了后面进行在*之前的路径下寻找类
	...
	/**
	 * 通过 ClassLoader 查找具有给定路径的所有类位置资源。
	...
	/**
	 * 通过 Ant 风格的 PathMatcher 查找与给定location pattern匹配的所有资源。
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		// 这里其实取到*之前的路径，因为*可以代表类名或者包名，这里要进行区分
		String rootDirPath = determineRootDir(locationPattern);
		// 这里其实取到*以及之后的路径
		String subPattern = locationPattern.substring(rootDirPath.length());
		// 取到全路径 /Users/xupan/Documents/...../lifecycle/service/
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			if (rootDirUrl.getProtocol().startsWith(ResourceUtils.URL_PROTOCOL_VFS)) {
				...
				// 这里把路径下的.class(包括子包下的.class)全部找了出来               **/*.class
